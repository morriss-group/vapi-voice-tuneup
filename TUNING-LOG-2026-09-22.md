...
The fix is in the tools server, not the prompt. Every email is normalized before it touches the CRM: whitespace stripped, spoken "at" and "dot" folded into symbols, lowercased, validated. If it still isn't an address, the customer is created without one and the tool's reply says so in words the agent speaks verbatim: *"The email address didn't come through clearly, so your confirmation will come by text to your cell instead. The test for it lives in the shop's private booking service, against a stand-in for the CRM, with the exact address that failed; the public skeleton here carries the same normalizer with invented fixtures (`tools-server/test/`).
...
## Later the same day: what one clean booking call still got wrong

*Same line, same day, written that evening. The call booked correctly. The owner read the transcript anyway and found four things wrong with what the agent said, and fixing them turned up a fifth on the backup line. All five went live at 5:12 PM after a text test against the real tools. Deploy and rollback are the same one-line prompt change with a snapshot on each side.*

1. **The agent invented the owner's surname.** The caller asked for it at the end of the booking. The prompt had never contained it, so the model produced a plausible name and said it flatly. Asked the same thing in text mode, the same prompt refused instead. Both are the same hole. The morning's "use the tool's words exactly" rule only disciplines what a tool returned; it says nothing about a question no tool answers. **Fix:** the owner's name in the prompt, and a general rule: if it is not in these instructions, you do not know it; say "I don't have that in front of me, but the owner can tell you," and never guess a name, number, date, price, or policy. Tested: the name comes back right, a question about the owner's truck is handed off, and the diagnostic fee is still quoted, which was the over-firing case to watch.
2. **A hyphen next to a digit is spoken as arithmetic.** The read-back rule's own example is hyphenated, so the model wrote `E-4-2` and the voice said "E minus four minus two." Four attempts to land a nine-character address on a patient caller. This is almost certainly the "s came through as minus" from the morning's entry as well. **Fix:** digits inside an address are spoken as words with ellipses, and a hyphen never touches a digit: "B-U-D-C-A-S-T-L-E... four... two at gmail dot com."
3. **A neutral question got a menu.** The rule says ask "And what neighborhood is that?" and stop. The agent appended "the X or Y side of things?" naming the two covered neighborhoods, then said "that sounds like X to me." The caller agreed. **Fix:** ask it in exactly those words, no list for the caller to pick from. The text test had passed this rule both before and after; a model follows a rule at a rate, not a switch, and the wording has to leave no room.
4. **Dead air after goodbye.** There was no goodbye rule, so the agent said "bye" and waited. Thirteen seconds later the idle message fired ("Take your time, I'm right here whenever you're ready") and the caller said "I was hanging up." **Fix:** when the caller says goodbye or that they are all set, one short goodbye and the end-call function in the same turn.
5. **The backup line could not hang up at all.** Its prompt told it to end robocalls silently, and it had been ported that way weeks earlier. The Retell LLM had no `end_call` tool in `general_tools`. Nothing in a prompt can hang up a call without the tool. Added.

**Why the hourly reviewer did not ring the phone.** It caught items 1 and 2 within eighteen minutes and wrote the same two fixes, then stayed silent, because its alert rules are about outcomes: a lost customer, a callback request, a fee dispute. A defect inside a call that booked fine is "log only." An outcome reviewer will never alert on a rule broken during a call that ended well. If you run one, give it your rulings as a checklist and one alert category for a rule broken on a live call.
